...
namespace gcp
{
   ...
   {
      GCP_Rect(){};
      GCP_Rect(const GCP_Point<T> &topLeft_, const GCP_Point<T> &widthHeight) :topLeft(topLeft_), widthHeight(bottomRight_){}
   ...
   public:
      ...
      {
         GCP_Point<T> TopLeft(std::fmin(line.pointA.X, line.pointB.X), std::fmin(line.pointA.Y, line.pointB.Y));
         GCP_Point<T> BottomRight(std::fmax(line.pointA.X, line.pointB.X), std::fmax(line.pointA.Y, line.pointB.Y));
